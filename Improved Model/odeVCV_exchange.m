...
global VD Do13 Dc13 Do Dc Solubility_O2 Solubility_CO2 Vpc Th L KT KR L2 h r2 AcceRate Ps Ts Tbody fo fc exHold1 exHold2  T Ti Thold KT_co2 KR_co2 
...
Af_co2 = KT_co2.*Solubility_CO2.*y(2);
Bf_co2 = KR_co2.*Solubility_CO2.*y(2);
df_dPco2 = (KR_co2*Solubility_CO2*(Bf_co2 + 1)^3 + 3*KR_co2*Solubility_CO2*Bf_co2*(Bf_co2 + 1)^2 + KT_co2*L*Solubility_CO2*(Af_co2 + 1)^3 + 3*KT_co2*L*Solubility_CO2*Af_co2*(Af_co2 + 1)^2)/((Bf_co2 + 1)^4 + L*(Af_co2 + 1)^4)...
          - ((Bf_co2*(Bf_co2 + 1)^3 + L*Af_co2*(Af_co2 + 1)^3)*(4*KR_co2*Solubility_CO2*(Bf_co2 + 1)^3 + 4*KT_co2*L*Solubility_CO2*(Af_co2 + 1)^3))/((Bf_co2 + 1)^4 + L*(Af_co2 + 1)^4)^2;
...
dy(2) = Dc13.*(1 + 4.*Th.*df_dPco2./Solubility_CO2)^(-1).*(y(9) - y(2))./(Solubility_CO2.*Vpc) + AcceRate.*L2.*h.*y(3)./Solubility_CO2 - AcceRate.*r2.*y(2);
...
if mod(t,T) <= (Ti + Thold) && (t <= exHold1 || t > exHold2) 
    ...
    dy(4) = Q.*(y(4) - y(5))./VD;                                                      %dPD_O2,mmHg
    dy(5) = (Q.*y(5) - Qp.*y(6) - y(5).*dVc_dt)./Vc;                                   %dPc_O2,mmHg
    dy(6) = (Qp.*y(6) - y(6).*dVpa_dt - Ps.*Tbody.*Do.*(y(6) - y(1))./Ts)./Vp;         %dPA_O2,mmHg
    
    dy(7) = Q.*(y(7) - y(8))./VD;                                                      %dPD_CO2,mmHg
    dy(8) = (Q.*y(8) - Qp.*y(9) - y(8).*dVc_dt)./Vc;                                   %dPc_CO2,mmHg
    dy(9) = (Qp.*y(9) - y(9).*dVpa_dt - Ps.*Tbody.*Dc.*(y(9) - y(2))./Ts)./Vp;         %dPA_CO2,mmHg
end
...
%     dy(7) = Dc.*(y(14) - y(7))./(Solubility_CO2.*Vpc) + AcceRate.*L2.*h.*y(8)./Solubility_CO2 - AcceRate.*r2.*y(7);    % CO2 partial pressure in blood
%     dy(8) = AcceRate.*r2.*Solubility_CO2.*y(7) - AcceRate.*L2.*h.*y(8);                % z
%     dy(9) = Q.*(y(9) - y(10))./VD;                                                     %dPD_O2,mmHg
%     dy(10) = (Q.*y(10) - Qp.*y(11) - y(10).*y(4))./Vc;                                 %dPc_O2,mmHg
%     dy(11) = (Qp.*y(11) - y(11).*y(5) - Ps.*Tbody.*Do.*(y(11)-y(6))./Ts)./Vp;          %dPA_O2,mmHg
%     dy(12) = Q.*(y(12) - y(13))./VD;                                                   %dPD_CO2,mmHg
%     dy(13) = (Q.*y(13) - Qp.*y(14) - y(13).*y(4))./Vc;                                 %dPc_CO2,mmHg
%     dy(14) = (Qp.*y(14) - y(14).*y(5) - Ps.*Tbody.*Dc.*(y(14)-y(7))./Ts)./Vp;          %dPA_CO2,mmHg
